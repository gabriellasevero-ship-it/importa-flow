...
/** Página de conteúdo do PDF pronta para extração por IA (imagem + texto nativo). */
export type CatalogPageForAi = {
  /** Índice 0-based da página no PDF. */
  pageIndex: number;
  /** JPEG da página renderizada (para recorte/upload). */
  blob: Blob;
  /** Mesmo JPEG em base64 (sem o prefixo data:), para enviar à IA. */
  base64: string;
  mimeType: string;
  /** Texto nativo do PDF (mais confiável que OCR). */
  nativeText: string;
  width: number;
  height: number;
};

export type ExtractCatalogPagesForAiOptions = {
  /** Disparado ao concluir a leitura de cada página do PDF. */
  onPageProcessed?: (progress: { currentPage: number; totalPages: number }) => void;
};
...
/**
 * Renderiza cada página de conteúdo do PDF (pula capa/promoção) em JPEG e devolve
 * imagem (blob + base64) e texto nativo, para extração por IA de visão.
 */
export async function extractCatalogPagesForAi(
  file: File,
  options?: ExtractCatalogPagesForAiOptions
): Promise<{
  pages: CatalogPageForAi[];
  skippedPageIndices: number[];
  totalPages: number;
  /** Texto das páginas iniciais puladas (capa/divisórias) onde costuma estar a categoria/tema. */
  coverText: string;
}> {
  const arrayBuffer = await file.arrayBuffer();
  const pdf = await pdfjsLib.getDocument({ data: arrayBuffer }).promise;
  const numPages = pdf.numPages;
  const pages: CatalogPageForAi[] = [];
  const skippedPageIndices: number[] = [];
  const coverTexts: string[] = [];
  let seenContentPage = false;
  ...
  return { pages, skippedPageIndices, totalPages: numPages, coverText };
}
